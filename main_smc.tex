...
The validation is fair at the infrastructure level but intentionally not identical at context construction: giving classical RAG the planner, diagnoser, and rubric outputs would convert it into the proposed framework, while withholding them from \textsc{MARLET} would remove the claimed contribution. We therefore report quality and resource outcomes together.
...
\begin{table}[t]
\caption{27B matched-slice replication ($\Delta=$Ours--RAG).}
\label{tab:validation_27b}
\centering
\scriptsize
\setlength{\tabcolsep}{2.2pt}
\renewcommand{\arraystretch}{0.98}
\begin{tabular}{@{}llp{0.42\columnwidth}p{0.32\columnwidth}@{}}
\toprule
Data & $n$ & Quality $\Delta$ & Resource $\Delta$ \\
\midrule
TutorEval & 200 & F1 $-0.000$, hit $+0.001$, rub. $-0.002$ & latency $-11.8$ s$^{\star\star\star}$, tokens $-992^{\star\star\star}$, calls $-0.56^{\star\star\star}$ \\
EduBench & 87 & EB12D $+0.032^{\star\star}$, rub. $+0.210^{\star\star\star}$, align $-0.063$ & latency $-22.1$ s$^{\star\star\star}$, tokens $-1861^{\star\star\star}$, calls $-0.57^{\star\star\star}$ \\
\bottomrule
\end{tabular}
\begin{minipage}{\columnwidth}\scriptsize\vspace{0.1em}$^{\star\star\star}p{<}10^{-4}$, $^{\star\star}p{<}0.01$. Latency $\Delta$ is wall-clock seconds/sample on the 4090 server.\end{minipage}
\end{table}
...
These results clarify the intended use of \textsc{MARLET}: it is not a universal replacement for retrieval, but a controller for deciding what grounding a tutoring sample needs. Chapter-grounded samples still need evidence, whereas learner-profile, rubric, or scenario-heavy samples may be better served by learner-state and answer-criteria summaries than by mandatory topical retrieval.
