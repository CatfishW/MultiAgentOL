...
Educational tutoring is more complex than factual question answering. A student's question is often only a partial signal: the appropriate response may depend on level, goals, weak points, recent confusion, dialogue, and likely misconceptions. We call this the learner's \emph{personalized learning state}. Therefore, tutoring retrieval must find evidence that is pedagogically appropriate for the learner, not merely topical.

One workaround is to append learner state, dialogue, grading criteria, and task constraints to the retrieval query. However, this shifts the burden onto one query embedding that must compress topic, level, misconception, instructional goal, and response constraints. This can blur search intent and return evidence that satisfies some needs while missing others.
...
\bottomrule
\end{tabular}
\begin{minipage}{\columnwidth}\footnotesize\vspace{0.2em}$^{\star\star\star}p{<}10^{-4}$, $^{\star\star}p{<}0.01$, $^{\star}p{<}0.05$. Runtime is mean wall-clock seconds per sample from paired 4B runs on the 4090 server.\end{minipage}
\end{table}
