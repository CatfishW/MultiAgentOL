...
\begin{table}[t]
\caption{4B quality and 4090 resource summary. Quality is higher-better; runtime, tokens, and calls are lower-better.}
...
\bottomrule
\end{tabular}
\begin{minipage}{\columnwidth}\scriptsize\vspace{0.1em}$^{\star\star\star}p{<}10^{-4}$, $^{\star\star}p{<}0.01$, $^{\star}p{<}0.05$. Runtime is mean wall-clock seconds on the 4090 server.\end{minipage}
\end{table}

\begin{table}[t]
\caption{27B matched-slice quality and resource summary (TutorEval $n{=}200$, EduBench $n{=}87$).}
...
Data & Metric & RAG & Ours & $\Delta$ \\
\midrule
\multirow{6}{*}{TutorEval} & Token-F1 & \textbf{0.1273} & 0.1271 & $-0.0002$ \\
& TE hit & 0.9964 & \textbf{0.9976} & $+0.0012$ \\
& Rubric & \textbf{0.9973} & 0.9957 & $-0.0017$ \\
& Latency (s) & 39.33 & \textbf{27.49} & $-11.84^{\star\star\star}$ \\
& Tokens & 4053 & \textbf{3061} & $-992^{\star\star\star}$ \\
& Calls & 1.86 & \textbf{1.31} & $-0.56^{\star\star\star}$ \\
\midrule
\multirow{6}{*}{EduBench} & EB12D & 0.411 & \textbf{0.443} & $+0.032^{\star\star}$ \\
& Rubric & 0.745 & \textbf{0.955} & $+0.210^{\star\star\star}$ \\
& EduAlign & \textbf{0.316} & 0.253 & $-0.063$ \\
& Latency (s) & 40.17 & \textbf{18.10} & $-22.07^{\star\star\star}$ \\
& Tokens & 3967 & \textbf{2107} & $-1861^{\star\star\star}$ \\
& Calls & 1.98 & \textbf{1.40} & $-0.57^{\star\star\star}$ \\
\bottomrule
\end{tabular}
\begin{minipage}{\columnwidth}\scriptsize\vspace{0.1em}$^{\star\star\star}p{<}10^{-4}$, $^{\star\star}p{<}0.01$. Higher/lower follows Table~\ref{tab:validation_summary}; runtime is seconds/sample on the 4090 server.\end{minipage}
\end{table}

\subsection{Trade-offs and Ablations}

For TutorEval, 95\% CIs are $[0.0002,0.007]$ for Token-F1, $[0.006,0.032]$ for tutor-hit, and $[0.006,0.044]$ for rubric coverage. For EduBench, CIs are $[0.027,0.036]$ for EB12D, $[0.174,0.198]$ for rubric, and $[-0.069,-0.011]$ for EduAlign; cost CIs are $[-6702,-5248]$ ms, $[-1847,-1741]$ tokens, and $[-0.52,-0.46]$ calls. Classical RAG keeps a small EduAlign edge, but the frozen hybrid skips retrieval on every EduBench sample, so many scenario-heavy prompts are better served by learner- and rubric-centered coordination than by external lookup.

The framework can use more modules than classical RAG, so resource cost must be reported. Here, scoped planner queries and the retrieval gate reduce token use. Two 4B ablations on a shared TutorEval slice ($n{=}100$) isolate the mechanism: forcing retrieval hurts Token-F1 ($-0.003$), tutor-hit ($-0.015$), and rubric coverage ($-0.025$), while disabling the shared critic shows that gains do not come mainly from critic revision. The 27B replication shows the same resource pattern: TutorEval quality is essentially tied, while EduBench keeps the EB12D/rubric advantage and EduAlign trade-off. Thus \textsc{MARLET} is not a universal replacement for retrieval; it is a controller for deciding what grounding a tutoring sample needs.
