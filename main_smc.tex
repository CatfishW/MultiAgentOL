...
\begin{table}[t]
\caption{27B paired quality and resource summary. TutorEval uses $n{=}400$ paired samples; EduBench uses $n{=}328$ unique paired rows after duplicate public IDs in the 400-example slice are collapsed.}
...
\bottomrule
\end{tabular}
\begin{minipage}{\columnwidth}\scriptsize\vspace{0.1em}$^{\star\star\star}p{<}10^{-4}$, $^{\star\star}p{<}0.01$, $^{\star}p{<}0.05$. Higher/lower follows Table~\ref{tab:validation_summary}; runtime is seconds/sample on the 4090 server.\end{minipage}
\end{table}
