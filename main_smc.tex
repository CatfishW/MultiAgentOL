...
Classical RAG can be manually enriched by appending learner state, dialogue history, grading criteria, and task constraints to the query. The difficulty is that a single retrieval query must then do several jobs at once: represent the topic, learner level, suspected misconception, instructional goal, and response constraints. This can blur search intent, retrieve evidence for the wrong instructional level, and make it hard to inspect why retrieval was or was not used.

We propose \textsc{MARLET} (Multi-Agent Retrieval for Learning-State-Aware Educational Tutoring) as an augmented RAG framework for this setting. Instead of letting the retriever act as the top-level controller, a supervisory router first identifies the tutoring regime and decides whether retrieval is needed. A planner scopes the search intent, a diagnoser summarizes visible learning state, and a rubric module states what the answer must satisfy. These modules run before the fixed tutor writes, so the retriever becomes a conditional tool inside an inspectable tutoring controller rather than a mandatory first step.
...
Educational tutoring includes explanation, misconception diagnosis, rubric-based scoring, and next-step feedback. These subproblems require different grounding signals, so retrieval recall alone is not enough~\cite{macina2025mathtutorbench,srinivasa2025tutorbench}. Our validation on TutorEval and EduBench shows that the same tutor scores higher when fed context assembled by \textsc{MARLET}; on EduBench it also reduces latency and backbone calls.
